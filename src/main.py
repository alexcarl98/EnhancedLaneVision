...
def color_thresholding(image, color_masks, rect_mask):
    ...
    # rect_mask = remove_background_morph_demo(image)
...
class PreProcessingMask():
    ...
    def __init__(self,cap:cv2.VideoCapture, yellow_r:HsvColorRange=None, 
...
def run(name, prp_mask):
    ...
    while(cap.isOpened()):
        ...
        if frame is None:
            break
        thresh_img = color_thresholding(frame,prp_mask.get_color_masks(), prp_mask.recty)
